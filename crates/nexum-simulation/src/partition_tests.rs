...
fn outbound_budget_fails_the_tick_atomically() {
    let config = SimulationConfig::new().with_max_messages_per_tick(1);
...
fn handler_world(config: SimulationConfig) -> World {
    let mut world = fixture(config);
    world
        .native_mut()
        .register(
...
fn delivered_messages_run_handlers_in_deterministic_batch_order() {
    let mut world = handler_world(SimulationConfig::new());
...
fn parallel_mode_produces_identical_outbound_traces() {
    let run = |config: SimulationConfig| -> Vec<PartitionMessage> {
        let mut world = fixture(config);
        for id in 0..3u64 {
            world
                .add_system(
