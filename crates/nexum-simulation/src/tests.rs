...
fn event_budget_is_enforced() {
    let config = SimulationConfig::new().with_max_events_per_tick(1);
...
fn over_limit_frame_is_rejected() {
    let config = SimulationConfig::new().with_max_commands_per_frame(1);
    let mut world = fixture(config);
    let mut frame = InputFrame::new(TickId::from_u64(0));
...
fn determinism_trace(seed: u64) -> SimulationTrace {
    let mut world = fixture(SimulationConfig::new().with_seed(seed));
    world
        .add_system(
...
fn rng_values_are_deterministic_across_runs() {
    let mut a = fixture(SimulationConfig::new().with_seed(7));
    let mut b = fixture(SimulationConfig::new().with_seed(7));
