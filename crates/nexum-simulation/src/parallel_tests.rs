...
use crate::{ExecutionMode, SimulationConfig, World};

/// A world with three single-column tables `a`, `b`, `c` (ids 0, 1, 2).
fn fixture(config: SimulationConfig) -> World {
    ...
    World::new(WorldId::from_u64(0), store, config).unwrap()
}
...
fn trace_of(world: &mut World, ticks: u64) -> Trace {
...
fn rich_world(config: SimulationConfig) -> World {
...
fn later_group_sees_earlier_groups_provisional_writes() {
    ...
    let (serial_dump, _) = run(SimulationConfig::new().with_execution(ExecutionMode::Serial));
    let (parallel_dump, _) =
        run(SimulationConfig::new().with_execution(ExecutionMode::Parallel(4)));
...
fn same_table_interleaved_writes_assign_identical_real_ids() {
    ...
    let run = |config: SimulationConfig| -> Vec<(RowId, Row)> {
        let mut world = fixture(config);
        world
            .add_system(
                SystemDefinition::with_access(
...
fn lying_write_declaration_is_detected_not_silently_wrong() {
    ...
    let mut serial = fixture(SimulationConfig::new().with_execution(ExecutionMode::Serial));
    serial
        .add_system(
            SystemDefinition::with_access(
                SystemId::from_u64(0),
    ...
    let mut parallel = fixture(SimulationConfig::new().with_execution(ExecutionMode::Parallel(2)));
    parallel
        .add_system(
            SystemDefinition::with_access(
                SystemId::from_u64(0),
...
fn first_failure_in_system_order_fails_the_tick_identically() {
    let run = |config: SimulationConfig| -> (Error, bool) {
        let mut world = fixture(config);
        world
            .add_system(
                SystemDefinition::with_access(
...
fn panicking_system_in_a_parallel_group_fails_atomically() {
    let run = |config: SimulationConfig| -> Error {
        let mut world = fixture(config);
        world
            .add_system(
                SystemDefinition::with_access(
...
fn wide_world(config: SimulationConfig) -> World {
    let mut store = TableStore::new();
...
fn one_hundred_systems_in_ten_groups_are_worker_count_independent() {
    let run = |config: SimulationConfig| -> Trace { trace_of(&mut wide_world(config), 2) };
