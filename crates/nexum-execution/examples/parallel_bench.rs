...
//!
//! Usage: `cargo run --release -p nexum-execution --example parallel_bench [ticks]`
//!
...
use nexum_execution::{
    ExecutionMode, InputFrame, Partition, PartitionConfig, SystemAccess, SystemDefinition,
};
...
type Build = fn(PartitionConfig, usize) -> Partition;

fn run(world: &mut Partition, ticks: u64) -> (u128, usize) {
...
fn bench(label: &str, ticks: u64, build: Build, count: usize) {
    let mut serial = build(
        PartitionConfig::new().with_execution(ExecutionMode::Serial),
        count,
    );
...
fn disjoint_world(config: PartitionConfig, count: usize) -> Partition {
...
fn groups_world(config: PartitionConfig, _count: usize) -> Partition {
...
fn heavy_world(config: PartitionConfig, count: usize) -> Partition {
    let names = wide_tables(count);
    let store = table_store(&names);
    let mut world = Partition::new(WorldId::from_u64(0), store, config).unwrap();
    for i in 0..count as u64 {
...
fn conflicting_world(config: PartitionConfig, count: usize) -> Partition {
...
fn mixed_world(config: PartitionConfig, _count: usize) -> Partition {
