//! [`ExecutionContext`]: the only surface a reducer or system sees.
//!
...
/// The controlled execution surface handed to a running system or reducer.
...
impl<'a> ExecutionContext<'a> {
    ...
    ///
    /// Constructed by [`crate::Partition`] for every system, every tick. A
...
///
/// Shared by the system path (through [`ExecutionContext`]) and the
/// scheduled-event path (in [`crate::Partition`]), so every event that reaches a
