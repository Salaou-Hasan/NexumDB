//! Simulation configuration ([`PartitionConfig`], ADR-009).
...
/// The configuration of a partition: the deterministic RNG seed, the
/// execution mode, and the bounded-resource limits that keep a partition
/// (and its inputs, events, and schedule) from growing without bound
/// (ADR-009 D1).
///
/// The seed is part of the determinism contract: the same seed + inputs +
/// systems reproduce the same execution.
