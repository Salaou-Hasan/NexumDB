...
//! [`ExecutionContext`] that delegates to the tick's transaction.
//!
...
/// The native execute function of a system.
///
/// Higher-ranked so the context borrow can be fresh for every tick.
pub type SystemFn = for<'a> fn(&mut ExecutionContext<'a>, &InputFrame) -> Result<()>;
...
/// A registered system: identity + ordering + execute function.
#[derive(Debug, Clone)]
...
/// A registry of systems, always sorted by `(priority, id)`.
