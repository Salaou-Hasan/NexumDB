//! The [`Partition`] (formerly `World`): one authoritative execution
//! partition (ADR-009 D1, D2).
//!
//! A partition owns the authoritative [`TableStore`], the ordered system
...
//! # use nexum_execution::{InputCommand, InputFrame, PartitionConfig, SystemDefinition, Partition};
...
use crate::config::{ExecutionMode, PartitionConfig};
...
/// observational  never influences simulation semantics.
...
/// One authoritative execution partition: a table store plus its own tick
/// loop, event schedule, and reducer registries (ADR-009 D1, ADR-012 D1).
///
/// A partition is the database's execution unit — every partition processes
/// its queued reducer inputs each tick, generating a deterministic commit.
/// (Formerly named `Partition`; retained as a deprecated alias below.)
#[derive(Debug)]
pub struct Partition {
    ...
    /// the partition config; never influences semantics.
...
impl Partition {
    /// Creates a partition owning `store`, with `config` bounds validated.
    pub fn new(id: WorldId, store: TableStore, config: PartitionConfig) -> Result<Self> {
        config.validate()?;
    ...
    /// unless profiling is enabled in the partition config.
    ...
    /// Advances execution by one tick (ADR-009 D2, D6) with no inbound
    ...
    /// Advances execution by one tick, delivering the inbound
    ...
    /// call batch, so every Phase 912 call site is unchanged.
    ...
    pub fn tick_with_calls(
        ...
        let result = (|| -> Result<()> {
            ...
            // eliminates the ~100 s absorb overhead per call by keeping
...
///
/// Deprecated: use [`Partition`] directly. Kept so existing callers port
/// incrementally during the World → Partition migration.
pub type World = Partition;
