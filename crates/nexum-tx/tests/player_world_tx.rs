...
fn join_match(store: &mut TableStore, match_id: u64, a: nexum_core::RowId, b: nexum_core::RowId, a_id: u64, b_id: u64, zone: u64) {
    let mut tx = Transaction::begin(store);
    // Read both players (records versions), then write all three tables.
    let player_a = tx.get(store, "players", a).unwrap().expect("player a exists");
    let player_b = tx.get(store, "players", b).unwrap().expect("player b exists");
    let (a_zone, b_zone) = (
    ...
    tx.insert(store, "matches", row![match_id, a_id, b_id, zone]).unwrap();
    tx.update(store, "players", a, row![a_id, zone + 1, 100i32, 5u32]).unwrap();
    tx.update(store, "players", b, row![b_id, zone + 1, 100i32, 6u32]).unwrap(); // distinct level: by_level is unique
...
fn matchmaking_world_commits_atomically_across_tables() {
    ...
    assert_eq!(
        store.table("players").unwrap().lookup("by_zone", &[Value::U64(10)]).unwrap().len(),
        2
    );
    ...
    assert!(matches.get_by_primary_key(&[Value::U64(100)]).unwrap().is_some());
}
...
fn matchmaking_conflict_aborts_every_table() {
    ...
    tx.get(&store, "players", nexum_core::RowId::from_u64(0)).unwrap();
    tx.insert(&store, "matches", row![50u64, 1u64, 2u64, 10u64]).unwrap();
    ...
    assert_eq!(
        store.table("players").unwrap().version_of(nexum_core::RowId::from_u64(0)),
...
fn unique_violation_in_one_table_rolls_back_all_writes() {
    ...
    let mut tx = Transaction::begin(&mut store);
    tx.insert(&store, "players", row![2u64, 20u64, 100i32, 1u32]).unwrap();
    tx.insert(&store, "matches", row![1u64, 1u64, 2u64, 20u64]).unwrap();
    ...
    assert!(store.table("matches").unwrap().is_empty());
    assert!(store.table("economy").unwrap().get_by_primary_key(&[Value::U64(2)]).unwrap().is_none());
...
fn economy_transfer_is_atomic() {
    ...
    tx.update(&store, "economy", from, row![1u64, 70i64]).unwrap();
    tx.update(&store, "economy", to, row![2u64, 230i64]).unwrap();
    let changes = tx.commit(&mut store).unwrap();

    assert_eq!(changes.len(), 2);
    assert!(changes.iter().all(|c| c.kind() == ChangeKind::Update));
    assert_eq!(
        store.table("economy").unwrap().get(from).unwrap().get_named(store.table("economy").unwrap().schema(), "coins"),
        Some(&Value::I64(70))
    );
    assert_eq!(
        store.table("economy").unwrap().get(to).unwrap().get_named(store.table("economy").unwrap().schema(), "coins"),
        Some(&Value::I64(230))
    );
}
