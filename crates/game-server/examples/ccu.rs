...
struct Args {
    ...
    density: usize,
    /// Deterministic seed for the player brains (same seed  identical
...
/// of correlated reducer calls  no per-call results, no pending tracking.
struct MoveCtx {
...
/// Phase 26: parallelized on the client pool  under saturation the queues
...
/// unit moves (RTS axis) on top  entity work is *in addition to* player ops.
struct Mix {
    move_p: u16,
    fire_p: u16,
    reload_p: u16,
...
fn drive_profile(profile: char, tick: u64, clients: &mut [SimClient], hz: u64, mv: &MoveCtx) {
    match profile {
        ...
        'E' => {
            ...
            // ((tick + i) % k) so bursts spread evenly across ticks  a
