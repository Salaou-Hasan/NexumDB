...
//! - scan result envelope at `out_ptr`: `[u32 status][u32 len][u64 count]
//!   [rows...]`, each row `[u64 row_id][u64 nvalues][value...]`, each value
//!   `[u8 tag][payload]`. The `players` table is all U64/I64 columns, so
//!   every value is exactly 9 bytes; value *k*'s payload of row *r* sits at
//!   `r + 17 + k*9`.
//!
//! The module never depends on host-call ordering across ops for
//! correctness: the scan result is fully consumed before any write/emit op
//! (which reuse the `out_ptr` envelope area).
...
const WAT: &str = r#"(module
  ...
  (data (i32.const 90100) "dead")
  (data (i32.const 90200) "recharging")
  (data (i32.const 90300) "out of ammo")
  (data (i32.const 90400) "not in arena")
  (data (i32.const 90500) "bad args")
  (data (i32.const 90600) "hit")
  (data (i32.const 90700) "kill")
  (data (i32.const 90800) "disconnected")
  (data (i32.const 90900) "schema mismatch")
  (data (i32.const 91000) "host op failed")
  ...
  (func (export "_nexum_reducer_run") (result i32)
    ;; ---- locals ----
    (local $p i32)          ;; cursor
    (local $n i32)          ;; scan row count
    (local $caller i64)     ;; the shooter (stamped __caller)
    ;; self row
    (local $self_found i32)
    (local $self_rid i64)
    ...
    (if (i64.ne (i64.load align=1 (local.get $p)) (i64.const 1))
      (then (return (call $reject (i32.const 90500) (i32.const 8)))))
    ...
    (if (i32.ne (i32.load align=1 (i32.const 16384)) (i32.const 0))
      (then (return (call $reject (i32.const 91000) (i32.const 14)))))
    (local.set $n (i32.wrap_i64 (i64.load align=1 (i32.const 16392))))
    (local.set $p (i32.const 16400))

    ;; ---- pass 1: find the shooter and capture its row ----
    (block $pass1_done
      (loop $pass1
        (br_if $pass1_done (i32.eqz (local.get $n)))
        (if (i64.ne (i64.load align=1 (i32.add (local.get $p) (i32.const 8))) (i64.const 11))
          (then (return (call $reject (i32.const 90900) (i32.const 15)))))
        (if (i32.eqz (local.get $self_found))
          (then
            (if (i64.eq (i64.load align=1 (i32.add (local.get $p) (i32.const 17))) (local.get $caller))
              (then
                (local.set $self_rid (i64.load align=1 (local.get $p)))
                (local.set $s0 (i64.load align=1 (i32.add (local.get $p) (i32.const 17))))
                (local.set $s1 (i64.load align=1 (i32.add (local.get $p) (i32.const 26))))
                (local.set $s2 (i64.load align=1 (i32.add (local.get $p) (i32.const 35))))
                (local.set $s3 (i64.load align=1 (i32.add (local.get $p) (i32.const 44))))
                (local.set $s4 (i64.load align=1 (i32.add (local.get $p) (i32.const 53))))
                (local.set $s5 (i64.load align=1 (i32.add (local.get $p) (i32.const 62))))
                (local.set $s6 (i64.load align=1 (i32.add (local.get $p) (i32.const 71))))
                (local.set $s7 (i64.load align=1 (i32.add (local.get $p) (i32.const 80))))
                (local.set $s8 (i64.load align=1 (i32.add (local.get $p) (i32.const 89))))
                (local.set $s9 (i64.load align=1 (i32.add (local.get $p) (i32.const 98))))
                (local.set $s10 (i64.load align=1 (i32.add (local.get $p) (i32.const 107))))
                (local.set $self_found (i32.const 1))))))
        ;; advance: 16 + nvalues * 9
        (local.set $p (i32.add (local.get $p)
          (i32.add (i32.const 16)
            (i32.mul (i32.wrap_i64 (i64.load align=1 (i32.add (local.get $p) (i32.const 8)))) (i32.const 9)))))
        (local.set $n (i32.sub (local.get $n) (i32.const 1)))
        (br $pass1)))

    ;; ---- validate the shooter ----
    (if (i32.eqz (local.get $self_found))
      (then (return (call $reject (i32.const 90400) (i32.const 11)))))
    (if (i64.eq (local.get $s5) (i64.const 0))
      (then (return (call $reject (i32.const 90100) (i32.const 4)))))
    (if (i64.eq (local.get $s10) (i64.const 0))
      (then (return (call $reject (i32.const 90800) (i32.const 12)))))
    (if (i64.gt_s (local.get $s7) (i64.const 0))
      (then (return (call $reject (i32.const 90200) (i32.const 10)))))
    (if (i64.le_s (local.get $s9) (i64.const 0))
      (then (return (call $reject (i32.const 90300) (i32.const 12)))))
    ...
    ;; ---- pass 2: find an alive target at the aim cell ----
    (local.set $n (i32.wrap_i64 (i64.load align=1 (i32.const 16392))))
    (local.set $p (i32.const 16400))
    (block $pass2_done
      (loop $pass2
        (br_if $pass2_done (i32.eqz (local.get $n)))
        (if (i32.eqz (local.get $t_found))
          (then
            (if (i64.ne (i64.load align=1 (i32.add (local.get $p) (i32.const 17))) (local.get $caller))
              (then
                (if (i64.ne (i64.load align=1 (i32.add (local.get $p) (i32.const 62))) (i64.const 0))
                  (then
                    (if (i64.eq (i64.load align=1 (i32.add (local.get $p) (i32.const 26))) (local.get $aimx))
                      (then
                        (if (i64.eq (i64.load align=1 (i32.add (local.get $p) (i32.const 35))) (local.get $aimy))
                          (then
                            (local.set $t_rid (i64.load align=1 (local.get $p)))
                            (local.set $t0 (i64.load align=1 (i32.add (local.get $p) (i32.const 17))))
                            (local.set $t1 (i64.load align=1 (i32.add (local.get $p) (i32.const 26))))
                            (local.set $t2 (i64.load align=1 (i32.add (local.get $p) (i32.const 35))))
                            (local.set $t3 (i64.load align=1 (i32.add (local.get $p) (i32.const 44))))
                            (local.set $t4 (i64.load align=1 (i32.add (local.get $p) (i32.const 53))))
                            (local.set $t5 (i64.load align=1 (i32.add (local.get $p) (i32.const 62))))
                            (local.set $t6 (i64.load align=1 (i32.add (local.get $p) (i32.const 71))))
                            (local.set $t7 (i64.load align=1 (i32.add (local.get $p) (i32.const 80))))
                            (local.set $t8 (i64.load align=1 (i32.add (local.get $p) (i32.const 89))))
                            (local.set $t9 (i64.load align=1 (i32.add (local.get $p) (i32.const 98))))
                            (local.set $t10 (i64.load align=1 (i32.add (local.get $p) (i32.const 107))))
    ...
    (drop (call $call_op (i32.const 6) (i32.sub (local.get $p) (i32.const 0))))
    ...
    (if (local.get $t_found)
      (then
        ...
        (drop (call $call_op (i32.const 6) (i32.sub (local.get $p) (i32.const 0))))
        ;; emit "hit" (shooter)
        (local.set $p (call $put_str (i32.const 0) (i32.const 90600) (i32.const 3)))
        ...
        (if (i64.eq (local.get $t3) (i64.const 0))
          (then
            (local.set $p (call $put_str (i32.const 0) (i32.const 90700) (i32.const 4)))
            (local.set $p (call $put_value_u64 (local.get $p) (local.get $t0)))
            (drop (call $call_op (i32.const 8) (local.get $p)))))))    (return (call $ret_i64 (local.get $damage)))
))
"#;
