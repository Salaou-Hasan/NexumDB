...
At 8K clients x 1 pending call each, a movement tick performed ~64M predicate
...
Measured: inbound phase **25.5 ms → 2.3 ms avg** (11x).
...
- **The parallel tick works.** The serial tick phase for 8 worlds x 1000
...
- **Parallel efficiency is bounded by the non-parallel remainder.** A
  ...
  slices are single-threaded and O(clients). Hence p95 improves ~2x, not 8x.
...
| CCU | Profile | PxW | p50 | p95 | p99 | Class |
|----:|---------|-----|----:|----:|----:|-------|
| 10K | A (conn) | 8x8 | 10.0 ms | 11.5 ms | 12.1 ms | **PASS** |
| 15K | A (conn) | 8x8 | 17.0 ms | 18.4 ms | 19.3 ms | **PASS** |
| 20K | A (conn) | 8x8 | 25.4 ms | 27.3 ms | 32.0 ms | **PASS** |
| 10K | B (move) | 8x8 | 10.7 ms | 39.5 ms | 72.9 ms | DEGRADED |
| 15K | B (move) | 8x8 | 17.9 ms | 66.5 ms | 114.5 ms | SATURATED |
| 15K | B (move) | 16x16 | 21.5 ms | 64.6 ms | 97.6 ms | DEGRADED |
| 10K | C (real) | 8x8 | 11.5 ms | 156 ms | 815 ms | SATURATED (fire) |
| 15K | C (real) | 16x16 | 21.5 ms | 341 ms | 1.0 s | SATURATED (fire) |
...
budget). 16x16 marginally improves p99 (114 → 98 ms) but raises the
...
~8x vs the pre-Phase-18 serial ~5.5 s, but it remains the hard p99 spike.
...
only), 8 partitions x 8 workers, 20 Hz. The full stack — server **and**
...
```text
private ≈ 5.7 MB + 24.7 KB x CCU     (R² ≈ 1.0: 129/253/377/500 vs 131/251/376/502 MB)
working set ≈ 13 MB + 23.3 KB x CCU
```
...
wrong by ~12x and is now calibrated to the measured fit.

**Join-storm peak (operational caveat):** a mass connect/join without
client consumption spikes memory severalx — ~2.4x at 5K, ~4.7x at 10K,
~6.3x at 15K, ~8x at 20K (4.1 GB peak). The superlinear growth is the
