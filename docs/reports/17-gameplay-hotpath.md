...
| Benchmark config | hz=20, window=32, arena 48×24 |
...
With 500 players, that is 500 row checks *per reducer call* × 500
...
- `many_player_movement_indexed_pk_determinism` — 20 players × 50 ticks,
...
**Classification key:**
- PASS: p99 stays below 2× tick budget (100 ms).
- DEGRADED: p99 between 1× and 2× budget.
- SATURATED: p99 exceeds 2× budget.
...
change against every subscription — **O(changes × subscriptions)** per
tick. For 1000 clients all moving on the same tick:

> 1000 changes × 1000 subscriptions = **1,000,000 delta evaluations**
...
|---------|-----|----------------------|----------------------|-------------|
| D | 500 | 83 ms (SATURATED) | 2.7 ms | **30×** |
| D | 1,000 | 119 ms (SATURATED) | 13.6 ms | **8.7×** |
...
### What Phase 17 Fixed
1. **Game reducer O(N) scan** → direct PK/index lookup (30× server-side).
...
The subscription engine's O(changes × subs) delta evaluation is the
...
| `many_player_movement_indexed_pk_determinism` | game-server | 20 players × 50 ticks, same seed = same positions + change trace |
...
1. **Subscription all-to-all fan-out:** O(changes × subscriptions) per tick.
...
| Game reducer O(N) removed | ✅ server-side 30× improvement |
| TickUpdate encode-once | ✅ 17× encode cost reduction |
